#ifndef DEFINES_H_
#define DEFINES_H_

#include "ap_fixed.h"
#include "ap_int.h"
#include "nnet_utils/nnet_types.h"
#include <cstddef>
#include <cstdio>

// hls-fpga-machine-learning insert numbers
#define N_INPUT_1_1 80
#define N_INPUT_2_1 1
#define OUT_WIDTH_14 82
#define N_CHAN_14 1
#define N_OUTPUTS_2 80
#define N_FILT_2 8
#define N_OUTPUTS_2 80
#define N_FILT_2 8
#define OUT_WIDTH_15 82
#define N_CHAN_15 8
#define N_OUTPUTS_5 80
#define N_FILT_5 8
#define N_OUTPUTS_5 80
#define N_FILT_5 8
#define OUT_WIDTH_16 82
#define N_CHAN_16 8
#define N_OUTPUTS_8 80
#define N_FILT_8 8
#define N_OUTPUTS_8 80
#define N_FILT_8 8
#define N_OUTPUTS_17 80
#define N_FILT_17 1
#define N_OUTPUTS_11 80
#define N_FILT_11 1

// hls-fpga-machine-learning insert layer-precision
typedef nnet::array<ap_fixed<16,6>, 1*1> input_t;
typedef nnet::array<ap_fixed<16,6>, 1*1> layer14_t;
typedef ap_fixed<16,6> conv1d_13_accum_t;
typedef nnet::array<ap_fixed<16,6>, 8*1> layer2_t;
typedef ap_fixed<16,6> conv1d_13_weight_t;
typedef ap_fixed<16,6> conv1d_13_bias_t;
typedef nnet::array<ap_fixed<16,6>, 8*1> layer4_t;
typedef ap_fixed<18,8> activation_3_table_t;
typedef nnet::array<ap_fixed<16,6>, 8*1> layer15_t;
typedef ap_fixed<16,6> conv1d_14_accum_t;
typedef nnet::array<ap_fixed<16,6>, 8*1> layer5_t;
typedef ap_fixed<16,6> conv1d_14_weight_t;
typedef ap_fixed<16,6> conv1d_14_bias_t;
typedef nnet::array<ap_fixed<16,6>, 8*1> layer7_t;
typedef ap_fixed<18,8> activation_4_table_t;
typedef nnet::array<ap_fixed<16,6>, 8*1> layer16_t;
typedef ap_fixed<16,6> conv1d_15_accum_t;
typedef nnet::array<ap_fixed<16,6>, 8*1> layer8_t;
typedef ap_fixed<16,6> conv1d_15_weight_t;
typedef ap_fixed<16,6> conv1d_15_bias_t;
typedef nnet::array<ap_fixed<16,6>, 8*1> layer10_t;
typedef ap_fixed<18,8> activation_5_table_t;
typedef ap_fixed<16,6> conv1d_16_accum_t;
typedef nnet::array<ap_fixed<16,6>, 1*1> layer17_t;
typedef ap_fixed<16,6> conv1d_16_weight_t;
typedef ap_fixed<16,6> conv1d_16_bias_t;
typedef nnet::array<ap_fixed<16,6>, 1*1> result_t;
typedef ap_fixed<18,8> activation_6_table_t;

#endif
